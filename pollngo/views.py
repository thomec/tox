...
from pollngo.models import *
from pollngo.pforms import CreatePoll
from pollngo.pygooglechart import PieChart2D
...
def create(request):
    if request.method == 'POST':    
        form = CreatePoll(request, request.POST)
        if form.is_valid():
            question = form.save()
            try:
